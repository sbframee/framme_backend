...
router.post("/tagImages", upload.single('images'), async (req, res) => {
    try {
        ...
        let response
        if (data.length) {
            response = await Image.updateOne({ tag_uuid: value.tag_uuid, sort_order: value.sort_order }, value)
...
router.post("/tagText", async (req, res) => {
    try {
        ...
        console.log(value)
        // value = req.file ? { ...value, img_url: `/images/${req.file.originalname}` } : value
        let data = await Image.find({ tag_uuid: value.tag_uuid, user: value.user[0], sort_order: value.sort_order })
        let response
        if (data.length) {
            response = await Image.updateOne({ tag_uuid: value.tag_uuid, sort_order: value.sort_order }, value)
...
router.post("/postTags", async (req, res) => {
    ...
    try {
        ...
        if (response)
            ...
            res.json({ success: false, message: "Tags Not created" });
    } catch (err) {
        res.status(500).json({ success: false, message: err });
...
router.get("/getTags", async (req, res) => {
    ...
    try {
        ...
        if (data.length)
            ...
            res.json({ success: false, message: "Tags Not found" });
    } catch (err) {
        res.status(500).json({ success: false, message: err });
...
router.post("/getUserTags", async (req, res) => {
    ...
    try {
        ...
        for (let i of TagsData) {
            ...
            let response = await Image.find({ tag_uuid: i.tag_uuid, user: user_uuid })
            response = JSON.parse(JSON.stringify(response))
...
router.put("/putTags", async (req, res) => {
    try {
        ...
        if (response.acknowledged)
            ...
            res.json({ success: false, message: "Tags Not updated" });
    } catch (err) {
        res.status(500).json({ success: false, message: err });
